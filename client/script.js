...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/hello!').then((response) => {
        console.log(response.data)
        const repeatTextBtn = document.querySelector('#repeat-button')
        repeatTextBtn.textContent = response.data
        repeatTextBtn.style.display = 'block';
    })
...
const newQuery = () => {
    axios.get('http://localhost:3000/query-test?hello=display').then((res) => {
        console.log(res)
    })
}

document.querySelector('#query-button').addEventListener('click', newQuery)
